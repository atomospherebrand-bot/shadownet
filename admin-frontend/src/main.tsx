...
type TextItem = { key: string; content: string; imageUrl?: string | null };
...
const Settings = ({ settings, onChange, onSave, saving }: {
  ...
  <section className="card">
    ...
    <div className="form-grid">
      ...
      <label>
        CrystalPay shop id
        <input value={settings.crystalpayShopId || ''} onChange={(e) => onChange('crystalpayShopId', e.target.value)} />
      </label>
      <label>
        CrystalPay secret
        <input value={settings.crystalpaySecret || ''} onChange={(e) => onChange('crystalpaySecret', e.target.value)} />
      </label>
      <label>
        CrystalPay host
        <input value={settings.crystalpayHost || ''} onChange={(e) => onChange('crystalpayHost', e.target.value)} />
      </label>
      <label>
        Enot shop id
        <input value={settings.enotShopId || ''} onChange={(e) => onChange('enotShopId', e.target.value)} />
      </label>
      <label>
        Enot secret
        <input value={settings.enotSecret || ''} onChange={(e) => onChange('enotSecret', e.target.value)} />
      </label>
      <label>
        Enot host
        <input value={settings.enotHost || ''} onChange={(e) => onChange('enotHost', e.target.value)} />
      </label>
      <label>
        Bot token
...
const TextsCard = ({
  ...
  description: string;
  texts: TextItem[];
  onChange: (key: string, value: Partial<TextItem>) => void;
  onSave: () => void;
  saving: boolean;
  withImages?: boolean;
}) => (
    ...
    <div className="form-grid">
      {texts.map((text) => (
        <label key={text.key}>
          ...
          <textarea
            value={text.content}
            onChange={(e) => onChange(text.key, { content: e.target.value })}
            rows={3}
          />
          {withImages && (
            <div className="image-field">
              <input
                ...
                value={text.imageUrl || ''}
                onChange={(e) => onChange(text.key, { imageUrl: e.target.value })}
              />
              {text.imageUrl && (
                <div className="image-preview">
                  <img src={text.imageUrl} alt={text.key} />
                </div>
              )}
            </div>
          )}
...
const App = () => {
  ...
  const [bulkType, setBulkType] = useState('vless');
  const [bulkTariffId, setBulkTariffId] = useState<number | ''>('');
  ...
  const handleBulkImport = async () => {
    ...
    const items = lines.map((raw, idx) => ({
      rawUri: raw,
      type: bulkType || 'vless',
      tariffId: bulkTariffId === '' ? undefined : Number(bulkTariffId),
      label: bulkLabelPrefix ? `${bulkLabelPrefix} ${idx + 1}` : undefined,
      status: 'available',
    }));

    await fetch(`${apiUrl}/keys/bulk`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...authHeaders },
  ...
  const changeSiteText = (key: string, value: Partial<TextItem>) => {
    setSiteTexts((prev) => prev.map((t) => (t.key === key ? { ...t, ...value } : t)));
  };

  const changeBotText = (key: string, value: Partial<TextItem>) => {
    setBotTexts((prev) => prev.map((t) => (t.key === key ? { ...t, ...value } : t)));
  };
          ...
          <section className="card narrow">
            <div className="card-header">
              <div>
                ...
                <p className="meta">По умолчанию: admin / admin (из переменных окружения backend)</p>
              </div>
            </div>
            <div className="form-grid">
              <label>
            ...
            <section className="card">
              <div className="card-header">
                <div>
                  ...
                  <p className="meta">Вставьте список URI, выберите тип и тариф</p>
                </div>
                <button className="btn" onClick={handleBulkImport} disabled={isImporting}>
                  {isImporting ? 'Импортируем…' : 'Импортировать ключи'}
                </button>
              </div>
              <div className="form-grid">
                <label>
                ...
                <label>
                  ...
                  <input
                    ...
                    value={bulkTariffId}
                    onChange={(e) =>
                      setBulkTariffId(e.target.value === '' ? '' : Number(e.target.value))
                    }
                  />
                </label>
                <label>
