...
- Пользовательская регистрация/логин через JWT+cookie с ролями (`user`/`admin`) и эндпоинтами `/api/auth/register`, `/api/auth/login`, `/api/auth/me`, `/api/auth/logout`.
- Личный кабинет API: `/api/subscriptions/current` возвращает активную подписку, `/api/configs/user` — купленные конфиги пользователя.
- Базовые столбцы пользователей в БД дополнены email, хешем пароля и ролью; сиды настроены на новые контакты поддержки `support@shadownet.live` и `https://t.me/shadownet_live_bot`.
- Редактируемые тексты бота теперь поддерживают `imageUrl` (превью в админке, отправка картинки в /start у бота).
...
- Файлы `.env` не хранятся в репозитории: создайте их на своём окружении (`backend/.env`, `frontend-site/.env`, `admin-frontend/.env`, `telegram-bot/.env`) и пропишите PORT/DATABASE_URL/CORS_ORIGIN/JWT_SECRET/креды провайдеров/URL API согласно вашим настройкам.
- Дефолтный вход в админку: **admin / admin** (настраивается через `ADMIN_USER` и `ADMIN_PASSWORD` в `backend/.env`).
...
- Если ветка PR менялась вне Codex, создайте новый PR с текущего состояния (см. `docs/PR_WORKAROUND.md`).
...
- Личный кабинет на сайте подключён к backend-авторизации: регистрация/логин через cookies/JWT, загрузка активной подписки `/api/subscriptions/current` и своих конфигов `/api/configs/user`, логаут.
...
- Админка редактирует настройки (активный провайдер, shop id/secret/host CrystalPay и Enot, токен бота, ссылки и контакты поддержки) и тексты сайта/бота через эндпоинты `/api/settings` и `/api/settings/texts/*`.
...
- Массовый импорт ключей по списку URI (/api/keys/bulk) с выбором тарифа, типа и префикса метки; сразу пополняет инвентарь.
